...
fn test_remove() {
    ...
    for c in "bc".chars() {
        assert_eq!(l.remove(c, c as u32).is_some(), true);
    }
    assert_eq!(l.remove('a', 0).is_some(), false); // won't delete if score doesn't match
    assert_eq!(l.remove('z', 100).is_some(), false); // won't delete if element doesn't match
...
fn test_in_range() {
    ...
    assert_eq!(
        l.is_in_range(&(Bound::Excluded(3), Bound::Unbounded)),
        false
    );
}

fn first_in_range<T, S: PartialOrd, R: RangeBounds<S>>(
    list: &SkipList<T, S>,
    range: R,
) -> Option<(&T, usize)> {
...
fn last_in_range<T, S: PartialOrd, R: RangeBounds<S>>(
    list: &SkipList<T, S>,
    range: R,
) -> Option<(&T, usize)> {
